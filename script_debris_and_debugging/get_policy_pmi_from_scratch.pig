...
instance = LOAD '/user/sofias6/policy_religious_captures_docsnippets_nonoverlapping' USING PigStorage('\t') AS (searchterm:chararray,
                                                                                    text:chararray,
                                                                                    URL:chararray,
                                                                                    surt:chararray,
                                                                                    checksum:chararray,
...
corpus_counts = LOAD '/user/sofias6/lowercaseword_policydoccounts' USING PigStorage('\t') AS (term:chararray,
                                                                                  wordcount:long);
...
religious_cooccurring_wordcounts_with_prelim_pmi = FOREACH religious_cooccurring_wordcounts GENERATE word AS word,
                                                                                                     ...
                                                                                                     ((float) (religious_wordcount) / (float) (corpus_wordcount + 5)) AS pmi:float;

religious_cooccurring_wordcounts_with_prelim_pmi = ORDER religious_cooccurring_wordcounts_with_prelim_pmi BY pmi DESC;

STORE religious_cooccurring_wordcounts_with_prelim_pmi INTO 'prelim_pmi_alltime_policy_five/' USING PigStorage('\t');
